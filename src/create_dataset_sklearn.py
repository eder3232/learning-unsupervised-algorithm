...
def create_dataset(n_samples, case):
    if case == "aniso":
        ...
        X, y = datasets.make_blobs(
            n_samples=n_samples, cluster_std=[1.5, 3.5, 0.8], random_state=170
        )
    print(
        "Case: {}  Number of dimensions: {} Number of Samples: {}".format(
            case, X.shape[1], X.shape[0]
        )
    )
...
if __name__ == "__main__":
    ...
    list_dataset = [
        ["blobs", "varied_blobs1", "varied_blobs2"],
        ["aniso", "noisy_moons", "noisy_circles"],
    ]
    nsample = 1500
    nrow = 2
    ncol = 3
    ...
    for row in range(nrow):
        for col in range(ncol):
            ...
            ax[row, col].scatter(X[0, :], X[1, :], color=colormaps.jet(0), s=15)
